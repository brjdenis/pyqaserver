# A modified version of the picketfence module. Allowing for other kinds of MLCs
# Take from pylinac. version 2.2.7
# Denis Brojan, 2019
...
LEAF_TYPE = {"Varian_120": [-200.0, -190.0, -180.0, -170.0, -160.0, -150.0, -140.0, -130.0, -120.0, -110.0, -100.0, -95.0, -90.0, -85.0, -80.0, -75.0, -70.0, -65.0, -60.0, -55.0, -50.0, -45.0, -40.0, -35.0, -30.0, -25.0, -20.0, -15.0, -10.0, -5.0, 0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 70.0, 75.0, 80.0, 85.0, 90.0, 95.0, 100.0, 110.0, 120.0, 130.0, 140.0, 150.0, 160.0, 170.0, 180.0, 190.0, 200.0],
             ...
             "Elekta_80": [-200.0, -190.0, -180.0, -170.0, -160.0, -150.0, -140.0, -130.0, -120.0, -110.0, -100.0, -90.0, -80.0, -70.0, -60.0, -50.0, -40.0, -30.0, -20.0, -10.0, 0.0, 10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100.0, 110.0, 120.0, 130.0, 140.0, 150.0, 160.0, 170.0, 180.0, 190.0, 200.0],
             "Elekta_160": [-200.0, -195.0, -190.0, -185.0, -180.0, -175.0, -170.0, -165.0, -160.0, -155.0, -150.0, -145.0, -140.0, -135.0, -130.0, -125.0, -120.0, -115.0, -110.0, -105.0, -100.0, -95.0, -90.0, -85.0, -80.0, -75.0, -70.0, -65.0, -60.0, -55.0, -50.0, -45.0, -40.0, -35.0, -30.0, -25.0, -20.0, -15.0, -10.0, -5.0, 0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 70.0, 75.0, 80.0, 85.0, 90.0, 95.0, 100.0, 105.0, 110.0, 115.0, 120.0, 125.0, 130.0, 135.0, 140.0, 145.0, 150.0, 155.0, 160.0, 165.0, 170.0, 175.0, 180.0, 185.0, 190.0, 195.0, 200.0]
             }
...
class PicketFence:
    ...
    def analyze(self, tolerance: float=0.5, action_tolerance: float=None, hdmlc: bool=False, mlc_type="Varian_120", num_pickets: int=None,
        ...
        self.settings = Settings(self.orientation, tolerance, action_tolerance, hdmlc, mlc_type, self.image, self._log_fits)
        # adjust for sag
        if sag_adjustment != 0:
    ...
    def _add_leaf_error_subplot(self, ax: plt.Axes, fig):
...
class Settings:
    ...
    def __init__(self, orientation, tolerance, action_tolerance, hdmlc, mlc_type, image, log_fits):
        self.orientation = orientation
        self.tolerance = tolerance
        self.action_tolerance = action_tolerance
        self.hdmlc = hdmlc
        self.mlc_type = LEAF_TYPE[mlc_type]
        self.image = image
    ...
    def small_leaf_width(self) -> int:
        ...
        #leaf_width_mm = 5
        N = len(self.mlc_type)  #Added
        leaf_width_mm = self.mlc_type[N//2] - self.mlc_type[N//2-1]  # Added.
    ...
    def large_leaf_width(self) -> int:
        ...
        return (self.mlc_type[2]-self.mlc_type[1]) * self.dpmm

    @property
    def number_small_leaves(self) -> int:
        """The number of small leaves; depends on HDMLC status."""
        #return 40 if not self.hdmlc else 32
        leaves = self.mlc_type
        widths = [leaves[i]-leaves[i-1] for i in range(1, len(leaves), 1)]
        N = len(widths)
        nr_small = widths.count(widths[N//2])
        nr_large = widths.count(widths[1])
        if nr_small == nr_large:
            return nr_small//2
        else:
            return nr_small
    ...
    def number_large_leaves(self) -> int:
        ...
        return len(self.mlc_type) - 1 - self.number_small_leaves
    ...
    def leaf_centers(self) -> np.ndarray:
        ...
        # Added
        leaf_centers = []
        for p in range(0, len(self.mlc_type)-1, 1):
            leaf_centers.append((self.mlc_type[p] + self.mlc_type[p+1])/2.0)
        leaf_centers = np.asarray(leaf_centers) * self.dpmm
        leaf_centers -= leaf_centers[0]
        ...
        adjustment = leaf30_center - leaf_centers[(len(self.mlc_type)-1)//2-1]
...
class PicketManager:
    ...
    def error_hist(self) -> Tuple[List, ...]:
        ...
        num_mlcs = len(self.pickets[0].mlc_meas)
        for p in self.pickets:
            if len(p.mlc_meas) != num_mlcs:
                raise ValueError("Not all pickets have the same number of MLC leaves.")
...
class Picket:
    ...
    def find_mlc_peak(self, mlc_center):
        ...
        mlc_rows = np.arange(int(mlc_center) - self.sample_width, int(mlc_center) + self.sample_width + 1)
        if self.settings.orientation == UP_DOWN:
    ...
    def dist2cax(self) -> float:
        ...
        return (getattr(self.settings.image_center, axis) - getattr(p1, axis)) * self.settings.mmpd
...
class MLCMeas(Line):
    ...
    def leaf_pair(self) -> Tuple[int, int]:
        ...
        mlc_dist = (mlc_loc - epid_center) * self.settings.mmpd
       ...
       #         leaves[0] = 31 - leaf
        ...
        # Added
        leaves[0] = np.searchsorted(self.settings.mlc_type, mlc_dist)
        # set opposite leaf using an offset
        leaves[1] = 2*(len(self.settings.mlc_type)-1) - leaves[0]
        return leaves
