...
% if (couch_iso_dev_x*couch_iso_dev_x+couch_iso_dev_y*couch_iso_dev_y)**0.5 < couch_dist_tol:
%   couch_dist_pass = "Passed"
% else:
%   couch_dist_pass = "Failed"
% end
...
<ul class="nav nav-tabs nav-justified">
    ...
    <li>
        <a data-toggle="pill" href="#menu2">
            Analysis
            % if image_pass=="Failed" or coll_asym_pass=="Failed" or beam_dev_pass=="Failed" or couch_dist_pass=="Failed":
                <span class="label label-danger">Failed</span>
            % elif ("Failed" not in [image_pass, coll_asym_pass, beam_dev_pass, couch_dist_pass]) and ("Borderline" in [image_pass, coll_asym_pass, beam_dev_pass, couch_dist_pass]):
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">
                    ...
                    % if couch_dist_pass == "Failed":
                        <span class='label label-danger'>Failed</span>
                    % elif couch_dist_pass == "Passed":
                        <span class='label label-success'>Passed</span>
                    % end
                </h3>
            </div>
            <div class="panel-body">
                Spatial directions and distance:
            </div>
            <table class="table  table-borderless">
                <tr>
                    <td>
                ...
                <tr>
                    <td>
                        {{round(couch_iso_dev_x, 2)}} mm
                    </td>
                    <td>
                        {{round(couch_iso_dev_y, 2)}} mm
                    </td>
                    <td>
                        {{round((couch_iso_dev_x**2+couch_iso_dev_y**2)**0.5, 2)}} mm
                    </td>
                </tr>
    ...
    var machines_and_energies =  {{!save_results["machines_and_energies"]}};
        ...
        function save_to_database(){
            ...
            var json_data = {"User": "{{save_results["displayname"]}}",
                             ...
                             "TestType": "Gnt/coll + couch rotation",
                             ...
                             "CouchAxisLAT": "{{round(couch_iso_dev_x, 2)}}",
                             "CouchAxisLONG": "{{round(couch_iso_dev_y, 2)}}",
                             "CouchAxisDIST": "{{round((couch_iso_dev_x*couch_iso_dev_x+couch_iso_dev_y*couch_iso_dev_y)**0.5, 2)}}",
                             "WobbleCouch": "{{round(couch_wobble, 2)}}",
                             "Comment": $('#comment').val()
                            };
