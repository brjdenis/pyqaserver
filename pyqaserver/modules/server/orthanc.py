...
orthanc_bp = Blueprint('orthanc_calls', __name__,
...
def get_patients():
    ...
    pat_names_sorted, pat_ids_sorted, orthanc_ids_sorted = zip(
        *sorted(
            zip(pat_names, pat_ids, orthanc_ids),
            key=lambda i: i[0].casefold()
            )
        )
...
def get_studies(patient_orthanc_id):
    ...
    study_dates_srt, study_desc_srt, study_ids_srt, orthanc_ids_srt = zip(
        *sorted(
            zip(study_dates, study_desc, study_ids, orthanc_ids),
            key=lambda i: i[0]
            )
        )
...
def get_series(study_orthanc_id):
    ...
    series_dt_srt, series_desc_srt, series_num_srt, orthanc_ids_srt = zip(
        *sorted(
            zip(series_dt, series_desc, series_num, orthanc_ids),
            key=lambda i: i[0]
            )
        )
...
def get_instance_dt(instance_orthanc_id):
    address = db_general.Orthanc.get_address()
    auth = (address.user, address.password)
    uri = (f'http://{address.ip}:{address.port}'
...
def get_dt_from_instance(resp_json):
    ...
    try:
        datetime = dt.strptime(date_var.strip() + ' ' + time_var.strip()[0:6],
                               '%Y%m%d %H%M%S')
    except ValueError:
...
def get_instance_dt_and_label(instance_orthanc_id):
    ...
    try:
        ...
        label = 'Unknown'
    return {
        'datetime': get_dt_from_instance(resp),
        'label': label
        }
...
def get_instances(series_orthanc_ids):
    ...
    instance_datetime_srt = [i.strftime('%Y-%m-%d / %H:%M:%S')
                             for i in instance_datetime_srt]
    return {
...
def get_image_description(orthanc_instance_id):
    address = db_general.Orthanc.get_address()
    auth = (address.user, address.password)
    uri = (f'http://{address.ip}:{address.port}/instances/'
           f'{orthanc_instance_id}/content')
    headers = {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'Connection': 'close'
...
def go_to_orthanc_explorer():
    ...
    address = db_general.Orthanc.get_address()
    uri = (f'http://{address.ip}:{address.port}/app/explorer.html#series?uuid='
           f'{series_id}')
    return {
        'uri': uri
    }
